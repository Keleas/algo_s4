...
Выделяем из текущего графа компоненты сильной-связности. Обозначаем каждую
компоненту за отдельную вершину в сжатом графе.
Строим сжатый граф: если есть ребро от какой-то вершины из одной компоненты в
вершину в другой компоненте, то добавляем ребро между соответсвующими компонентами.
В итоге, считаем в сжатом графе количество вершин, из которых никуда нельзя пойти и
в которые нельзя попасть. Максимальное число между ними будет минимальным
количеством ребер, которое нужно добавить для построения сильно-связного графа.
